...
async function check() {
    const res = await fetch(`https://opendata.aemet.es/opendata/api/prediccion/especifica/municipio/horaria/11012/?api_key=${apiKey}`);
    const json = await res.json();
    if(json.datos) {
        const dataRes = await fetch(json.datos);
        const data = await dataRes.json();
        const dia = data[0].prediccion.dia[0];
        console.log("Cádiz - Estado del Cielo para hoy (" + dia.fecha + "):");
        dia.estadoCielo.forEach(c => console.log(`Hora: ${c.periodo}:00 -> ${c.value} - ${c.descripcion}`));
    } else {
        console.log("No data URL for Cadiz", json);
    }
}
check();
