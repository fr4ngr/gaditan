// destinosMap.js
// Lógica para inicializar y actualizar los mapas de las secciones de destinos utilizando MapLibre GL JS

const destinosMapManager = (() => {
    let maps = {
        aeropuertos: null,
        favoritos: null
    };
    let polylines = {
        aeropuertos: null,
        favoritos: null
    };
    let destinationMarkers = {
        aeropuertos: null,
        favoritos: null
    };
    
    // Cadiz center
    const originLat = 36.529;
    const originLon = -6.292;

    const initMap = (type) => {
        if (maps[type]) return maps[type];
        
        const mapElementId = `map-${type}`;
        const mapElement = document.getElementById(mapElementId);
        if (!mapElement || typeof maplibregl === 'undefined') return null;

        const mapInstance = new maplibregl.Map({
            container: mapElementId,
            style: {
                "version": 8,
                "sources": {
                    "osm": {
                        "type": "raster",
                        "tiles": ["https://basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}.png"],
                        "tileSize": 256,
                        "attribution": "© OpenStreetMap © CartoDB"
                    }
                },
                "layers": [
                    {
                        "id": "osm-layer",
                        "type": "raster",
                        "source": "osm",
                        "layout": { "visibility": "visible" }
                    }
                ]
            },
            center: [originLon, originLat],
            zoom: 13,
            attributionControl: false
        });
        
        mapInstance.addControl(new maplibregl.AttributionControl({ compact: true }), 'bottom-right');
        mapInstance.addControl(new maplibregl.NavigationControl({ showCompass: false }), 'bottom-right');

        // Añadir marcador de origen (Cádiz)
        const el = document.createElement('div');
        el.className = 'custom-div-icon';
        el.innerHTML = `<div style="display: flex; justify-content: center; align-items: center; filter: drop-shadow(0 4px 4px rgba(0,0,0,0.5));"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="#06b6d4" stroke="#111827" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"></path><circle cx="12" cy="10" r="3" fill="#111827"></circle></svg></div>`;
        
        new maplibregl.Marker({ element: el })
            .setLngLat([originLon, originLat])
            .addTo(mapInstance);

        maps[type] = mapInstance;
        return mapInstance;
    };

    const fetchRoute = async (type, destLat, destLon) => {
        try {
            // Limpiar ruta anterior
            if (polylines[type]) {
                if (maps[type].getLayer('route-outer-' + type)) maps[type].removeLayer('route-outer-' + type);
                if (maps[type].getLayer('route-inner-' + type)) maps[type].removeLayer('route-inner-' + type);
                if (maps[type].getLayer('route-animated-' + type)) maps[type].removeLayer('route-animated-' + type);
                if (maps[type].getSource('route-source-' + type)) maps[type].removeSource('route-source-' + type);
                polylines[type] = null;
            }
            if (destinationMarkers[type]) {
                destinationMarkers[type].remove();
                destinationMarkers[type] = null;
            }

            // OSRM espera lon,lat
            const url = `https://router.project-osrm.org/route/v1/driving/${originLon},${originLat};${destLon},${destLat}?overview=full&geometries=geojson`;
            const response = await fetch(url);
            if (!response.ok) throw new Error('Error fetching route');
            
            const data = await response.json();
            if (data.routes && data.routes[0]) {
                const route = data.routes[0];
                
                maps[type].addSource('route-source-' + type, {
                    type: 'geojson',
                    data: {
                        type: 'Feature',
                        properties: {},
                        geometry: route.geometry
                    }
                });

                // Línea exterior (borde oscuro)
                maps[type].addLayer({
                    id: 'route-outer-' + type,
                    type: 'line',
                    source: 'route-source-' + type,
                    layout: { 'line-join': 'round', 'line-cap': 'round' },
                    paint: {
                        'line-color': '#083344',
                        'line-width': 8,
                        'line-opacity': 0.6
                    }
                });

                // Línea interior sólida (Cyan)
                maps[type].addLayer({
                    id: 'route-inner-' + type,
                    type: 'line',
                    source: 'route-source-' + type,
                    layout: { 'line-join': 'round', 'line-cap': 'round' },
                    paint: {
                        'line-color': '#06b6d4',
                        'line-width': 5,
                        'line-opacity': 1.0
                    }
                });

                // Línea de flujo blanca
                maps[type].addLayer({
                    id: 'route-animated-' + type,
                    type: 'line',
                    source: 'route-source-' + type,
                    layout: { 'line-join': 'round', 'line-cap': 'round' },
                    paint: {
                        'line-color': '#ffffff',
                        'line-width': 3,
                        'line-opacity': 0.9,
                        'line-dasharray': [2, 6]
                    }
                });

                polylines[type] = true;

                const destEl = document.createElement('div');
                destEl.className = 'custom-div-icon';
                destEl.innerHTML = `<div style="display: flex; justify-content: center; align-items: center; filter: drop-shadow(0 4px 4px rgba(0,0,0,0.5));"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="#06b6d4" stroke="#111827" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"></path><circle cx="12" cy="10" r="3" fill="#111827"></circle></svg></div>`;

                destinationMarkers[type] = new maplibregl.Marker({ element: destEl })
                    .setLngLat([destLon, destLat])
                    .addTo(maps[type]);
                
                const bounds = new maplibregl.LngLatBounds();
                route.geometry.coordinates.forEach(coord => bounds.extend(coord));
                maps[type].fitBounds(bounds, { 
                    padding: { top: 150, left: 50, bottom: 100, right: 50 }, 
                    maxZoom: 14 
                });
            }
        } catch (error) {
            console.error('Error calculando ruta al destino:', error);
            // Limpieza por seguridad
            if (polylines[type]) {
                if (maps[type].getLayer('route-outer-' + type)) maps[type].removeLayer('route-outer-' + type);
                if (maps[type].getLayer('route-inner-' + type)) maps[type].removeLayer('route-inner-' + type);
                if (maps[type].getLayer('route-animated-' + type)) maps[type].removeLayer('route-animated-' + type);
                if (maps[type].getSource('route-source-' + type)) maps[type].removeSource('route-source-' + type);
                polylines[type] = null;
            }
            if (destinationMarkers[type]) {
                destinationMarkers[type].remove();
                destinationMarkers[type] = null;
            }
            
            // Fallback: solo mover el mapa y poner el pin
            const destEl = document.createElement('div');
            destEl.className = 'custom-div-icon';
            destEl.innerHTML = `<div style="display: flex; justify-content: center; align-items: center; filter: drop-shadow(0 4px 4px rgba(0,0,0,0.5));"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="#06b6d4" stroke="#111827" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"></path><circle cx="12" cy="10" r="3" fill="#111827"></circle></svg></div>`;
            
            destinationMarkers[type] = new maplibregl.Marker({ element: destEl })
                .setLngLat([destLon, destLat])
                .addTo(maps[type]);

            const bounds = new maplibregl.LngLatBounds();
            bounds.extend([originLon, originLat]);
            bounds.extend([destLon, destLat]);
            maps[type].fitBounds(bounds, { 
                padding: { top: 150, left: 50, bottom: 100, right: 50 }
            });
        }
    };

    const render = (dest, type) => {
        const mapInstance = initMap(type);
        if (!mapInstance) return;

        // Invalida tamaño para asegurar que MapLibre renderice correctamente después de un display:block
        setTimeout(() => {
            mapInstance.resize();
            
            if (dest.lat && dest.lon) {
                fetchRoute(type, dest.lat, dest.lon);
            }
            
            // Actualizar overlay info en el mapa
            const overlay = document.getElementById(`map-overlay-info-${type}`);
            if (overlay) {
                const badgeText = type === 'aeropuertos' ? 'DESDE CÁDIZ A' : 'DESDE CÁDIZ A';
                overlay.innerHTML = `
                    <div style="background: rgba(15, 23, 42, 0.75); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid rgba(255,255,255,0.1); border-radius: 9999px; padding: 0.85rem 1.25rem; box-shadow: 0 10px 30px rgba(0,0,0,0.3); display: flex; flex-direction: column; width: 100%; box-sizing: border-box;">
                        <div style="display: flex; align-items: center; gap: 0.85rem; width: 100%;">
                            <i data-lucide="map-pin" style="color: var(--brand-cyan); width: 28px; height: 28px; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.5)); flex-shrink: 0;"></i>
                            <div style="display: flex; flex-direction: column; flex: 1; min-width: 0; justify-content: center;">
                                <span style="color: var(--brand-cyan); font-size: 0.65rem; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 0.15rem;">${badgeText}</span>
                                <strong style="color: #fff; font-size: 1.1rem; font-weight: 800; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; line-height: 1.1;">${dest.name}</strong>
                                ${dest.time ? `<span style="color: rgba(255,255,255,0.7); font-size: 0.8rem; margin-top: 0.1rem;">Tiempo estimado: ${dest.time}</span>` : ''}
                            </div>
                        </div>
                    </div>
                `;
                overlay.style.display = 'block';
                if (typeof lucide !== 'undefined') lucide.createIcons();
                setTimeout(() => {
                    overlay.style.transition = 'all 0.8s cubic-bezier(0.1, 0.9, 0.2, 1) 0.4s';
                    overlay.style.transform = 'translateY(0)';
                    overlay.style.opacity = '1';
                }, 50);
            }
        }, 100);
    };

    return {
        render
    };
})();

// Exponer la función global para components.js
window.renderDestinoMap = destinosMapManager.render;
