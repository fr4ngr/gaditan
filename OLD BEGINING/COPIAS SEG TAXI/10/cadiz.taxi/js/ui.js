            ...
            try {
                ...
                if (data.features && data.features.length > 0) {
                    ...
                    data.features.forEach(feature => {
                        ...
                        const type = props.osm_value === "hospital" ? "🏥" : (props.osm_value === "beach" ? "🏖️" : "📍");
                        const displayName = `${type} <strong>${name}</strong> <span style="font-size:0.8em; color:var(--text-muted)">(${city || 'Cádiz'})</span>`;
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
        ...
        try {
            ...
            if (data.features && data.features.length > 0) {
                ...
                document.getElementById('calc-origin').value = street;
                ...
                document.getElementById('calc-origin').value = lat.toFixed(4) + ", " + lon.toFixed(4);
                calcContext.selectedOrigin = {
            ...
            document.getElementById('calc-origin').value = "Ubicación detectada";
            calcContext.selectedOrigin = {
                name: "Ubicación detectada",
                city: 'Cádiz',
                lat: lat,
