...
function renderDestino(dest, gridType) {
    ...
    <div class="mini-dest-card pildora-hover" onclick="showDestinoDetails('${dest.id}', '${gridType}')" style="margin-bottom: 1rem; cursor: pointer;">
        <div class="mini-dest-header" style="align-items: center; position: relative; width: 100%; display: flex; justify-content: space-between;">
            <div style="display: grid; grid-template-columns: 16px 1fr; gap: 0.15rem 0.4rem; align-items: center;">
                <div></div>
                <div style="font-size: 0.65rem; color: white; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; line-height: 1;">DESDE CÁDIZ A</div>
                <i data-lucide="${dest.icon}" size="16" style="color: var(--brand-cyan);"></i>
                <div style="line-height: 1.2;">${dest.name}</div>
            </div>
